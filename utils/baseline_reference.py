...
def copy_reference(image, output, base_name):
    ...
    sources = {"input_image": (image, f"{base_name}_input{image.suffix.lower()}")}
    ...
    if image.parent.name == "baseline_i2v" and metadata_path.is_file():
        ...
        sources.update({"ground_truth": (sample_dir / "gt.mp4", f"{base_name}_gt.mp4"),
                        "sample_metadata": (metadata_path, f"{base_name}_sample.json")})
    ...
    (output / f"{base_name}_reference.json").write_text(
